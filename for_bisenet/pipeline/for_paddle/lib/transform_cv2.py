...
class RandomResizedCrop(object):
    ...
    def __call__(self, im_lb):
        ...
        # print("intherandomsize")
        im_h, im_w, _ = im.shape
        sh, sw = np.random.random(2)
        sh, sw = int(sh * (im_h - crop_h)), int(sw * (im_w - crop_w))
        im=im[sh:sh+crop_h, sw:sw+crop_w, :]
        lb=lb[sh:sh+crop_h, sw:sw+crop_w]
        # print(im.shape)
        return dict(
            im=im.copy(),
            lb=lb.copy()
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, im_lb):
        # print("RandomHorizontalFlip",im_lb['im'].shape)
...
class ColorJitter(object):
    ...
    def __call__(self, im_lb):
        # print("ColorJitter",im_lb['im'].shape)
        im, lb = im_lb['im'], im_lb['lb']
        assert im.shape[:2] == lb.shape[:2]
...
class Compose(object):
    ...
    def __call__(self, im_lb):
        for comp in self.do_list:
            # print("finalllllll",comp)
            im_lb = comp(im_lb)
            # print("finalllllll",im_lb["im"].shape)
        return im_lb
